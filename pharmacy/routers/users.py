...
from pharmacy.database.models import cart_items
...
from pharmacy.dependencies.database import Database, AnnotatedUser, get_inventory_or_404
...
def get_curent_user(user:AuthenticatedUser)->User:
     return user



@router.post("/current/cart-items", response_model=CartItemSchema)
def add_cart_item_to_cart(user:AuthenticatedUser, cart_item_data: CartItemCreate, db: Database,)->CartItem:
...
def get_list_of_cart_items(user: AuthenticatedUser, db:Database)->list[CartItem]:
    return db.scalars(select(CartItem).where(CartItem.user_id)).all()
...
def get_list_of_orders(db: Database, user: AuthenticatedUser):
...
def delete_item_from_cart(user: AuthenticatedUser, db: Database, cart_item: AnnotatedUser)->None:
